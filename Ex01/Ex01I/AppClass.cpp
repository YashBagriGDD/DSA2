...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Yash Bagri yxb1236@rit.edu";
	...
	for (uint i = 0; i < 6; ++i)
	{
		m_lPosition.push_back( vector3(0.0f, 10.0f, 0.0f) );
...
void Application::Update(void)
{
	...
	for (uint i = 0; i < 6; ++i)
	{
		...
		matrix4 transformations = glm::translate(IDENTITY_M4, m_lPosition[i]) * glm::toMat4(m_lOrientations[i]);
		m_lModel[i]->SetModelMatrix(transformations);
		m_lRigidBody[i]->SetModelMatrix(transformations);
