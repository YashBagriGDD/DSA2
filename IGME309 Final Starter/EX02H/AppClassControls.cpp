...
// Continuous update (once per frame) for Discrete input use process events.
...
#pragma region Character Position
	...
	{
		m_pEntityMngr->ApplyForce(vector3(-5.0f * fDelta, 0.0f, 0.0f), "Cube");
	}
	...
	{
		m_pEntityMngr->ApplyForce(vector3(0.0f, 0.0f, -5.0f * fDelta), "Cube");
	}
...
void Application::ProcessKeyPressed(sf::Event a_event)
{
	...
	case sf::Keyboard::Space:
		m_sound.play();
		m_pEntityMngr->ApplyForce(vector3(0.0f, 3.0f, 0.0f), "Cube");
		break;
...
void Application::ProcessJoystickConnected(uint nController)
{
	...
	//m_bGUI_Controller = bConnected; //Open Controller Window if connected
	if (bConnected)
	{
